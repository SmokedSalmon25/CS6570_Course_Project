...
def sigmoid(x):
    x = np.asarray(x, dtype=np.float64)  # Ensure float64 for proper overflow checks
    return 1 / (1 + np.exp(-x))
...
def find_one_decision_boundary_point(ws, bs, layer_num=0, precision=10**(-8)):
    ...
    ms_p = binary_search(st=st, di=di, ws=ws, bs=bs,
                         ms_low=2 ** (-10), ms_high=2, precision=precision)
    ms_n = binary_search(st=st, di=di, ws=ws, bs=bs,
                         ms_low=-2, ms_high=-2 ** (-10), precision=precision)
    if ms_p is not None:
        point = st + di * ms_p

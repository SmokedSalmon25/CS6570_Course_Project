...
def extract_1_deep_nn(model_path=None, nn_shape=None, precision=10*(-4), l1_error=10*(-3)):
    ...
    # # Shuffle all weights
    # np.random.shuffle(weights_concat)

    # # Reconstruct original weight shapes
    # w1_shape = weights[0].shape  # (2, 784)
    # w2_shape = weights[1].shape  # (1, 2)

    # w1_size = np.prod(w1_shape)
    # w2_size = np.prod(w2_shape)

    # # Split and reshape
    # w1_new = weights_concat[:w1_size].reshape(w1_shape)
    # w2_new = weights_concat[w1_size:w1_size + w2_size].reshape(w2_shape)

    # # Put back into a list
    # weights_shuffled = [w1_new, w2_new]
    ...
    model_path = folder + 'relu_2_classes' + tmp_path
    # model_path = folder + 'relu' + tmp_path
    np.savez(model_path,
...
if __name__ == '__main__':
    ...
    nn_shape = [784, 2, 1]
    # nn_path = './models/simulated/relu_2_classes.npz'
    nn_path = './models/simulated/relu_2_classes.npz'

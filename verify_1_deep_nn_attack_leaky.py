...
def recover_gamma_p(x, ws, bs, ms=10**(-3), precision=10**(-8)):
    ...
    for j in range(d_0):
        ...
        di[j][0] = 1.0
        xi_p = x + di * ms
        _, yi_p = utils_leaky.f_cheat(x=xi_p, ws=ws, bs=bs)
        xi_n = x - di * ms
        _, yi_n = utils_leaky.f_cheat(x=xi_n, ws=ws, bs=bs)
        if yi_p == 1:
            ws_signs[j] = 1
        elif yi_n == 1:
            ws_signs[j] = -1
    ...
    _, y_index = utils_leaky.f_cheat(x=x_index, ws=ws, bs=bs)
    assert y_index == 1
    ...
    for j in range(1, d_0):
        ...
        di_j[j][0] = 1.0
        suitable_ms_p = utils_leaky.binary_search(st=x_index, di=di_j, ws=ws, bs=bs,
                                            ms_low=ms / 10000, ms_high=ms * 10000,
                                            precision=precision)
        suitable_ms_n = utils_leaky.binary_search(st=x_index, di=di_j, ws=ws, bs=bs,
...
def extract_1_deep_nn(model_path=None, nn_shape=None, precision=10**(-8), l1_error=10**(-3)):
    ...
    while cnt < n:
        ...
        x = utils_leaky.find_one_decision_boundary_point(ws=weights, bs=biases,
                                                   layer_num=1, precision=precision)
        if x is not None:
            cur_maps = utils_leaky.get_map(x=x, ws=weights, bs=biases)
            tmp_maps.append(cur_maps)
            tmp_boundary_points.append(x)

            soft_label, _ = utils_leaky.f_cheat(x=x, ws=weights, bs=biases)
        cnt += 1
    ...
    if len(gamma_p) != 0:
        unique_indexs = utils_leaky.filter_duplicate_vectors(arr=gamma_p, l1_error=l1_error)
    ...
    qw = np.log2(utils_leaky.oracle_query_times)
        ...
        for w_2_sign in [1, -1]:
            ...
            for i in unique_indexs:
                ...
                if np.any(hat_w_2 * w_2_sign < 0):
                    print('filtered by filter signs')
                    continue
                ...
                if flag == 0:
                    print('filtered by filter model_signatures')
                    continue
                ...
                prediction_matching_ratio = utils_leaky.check_prediction_matching_ratio(t_ws=weights,
                                                                                  t_bs=biases,
                                                                                  h_ws=cur_hat_ws,
                                                                                  h_bs=hat_bs)
    ...
    # model_path = folder + 'leaky_2' + tmp_path
    model_path = folder + 'leaky_1' + tmp_path
...
if __name__ == '__main__':
    ...
    nn_shape = [784, 2, 1]
    nn_path = './models/simulated/leaky_mnist_hidden2_1.npz'

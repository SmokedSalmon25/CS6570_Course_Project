...
def recover_gamma_p(x, ws, bs, ms=10**(-3), precision=10**(-8)):
    ...
    for j in range(d_0):
        ...
        di[j][0] = 1.0
        xi_p = x + di * ms
        _, yi_p = utils_tanh.f_cheat(x=xi_p, ws=ws, bs=bs)
        xi_n = x - di * ms
        _, yi_n = utils_tanh.f_cheat(x=xi_n, ws=ws, bs=bs)
        if yi_p == 1:
            ws_signs[j] = 1
        elif yi_n == 1:
            ws_signs[j] = -1
    ...
    _, y_index = utils_tanh.f_cheat(x=x_index, ws=ws, bs=bs)
    assert y_index == 1
    ...
    for j in range(1, d_0):
        ...
        di_j[j][0] = 1.0
        suitable_ms_p = utils_tanh.binary_search(st=x_index, di=di_j, ws=ws, bs=bs,
                                            ms_low=ms / 10000, ms_high=ms * 10000,
                                            precision=precision)
        suitable_ms_n = utils_tanh.binary_search(st=x_index, di=di_j, ws=ws, bs=bs,
...
def extract_1_deep_nn(model_path=None, nn_shape=None, precision=10*(-4), l1_error=10*(-3)):
    # load model
    ...
    while cnt < n:
        ...
        x = utils_tanh.find_one_decision_boundary_point(ws=weights, bs=biases,
                                                   layer_num=1, precision=precision)
        if x is not None:
            cur_maps = utils_tanh.get_map(x=x, ws=weights, bs=biases)
            tmp_maps.append(cur_maps)
            tmp_boundary_points.append(x)

            soft_label, _ = utils_tanh.f_cheat(x=x, ws=weights, bs=biases)
        cnt += 1
    tmp_boundary_point_num = len(tmp_boundary_points)
    print("Number of boundary points: ", tmp_boundary_point_num)
    print("Technically the maps here are: ", tmp_maps)
    ...
    if len(gamma_p) != 0:
        unique_indexs = utils_tanh.filter_duplicate_vectors(arr=gamma_p, l1_error=l1_error)
    ...
    qw = np.log2(utils_tanh.oracle_query_times)
        ...
        for w_2_sign in [1, -1]:
            ...
            for i in unique_indexs:
                ...
                if np.any(hat_w_2 * w_2_sign < 0):
                    print('filtered by filter unmatched_signs')
                    continue
                ...
                if flag == 0:
                  print(" Filtered: model signature mismatch in compare_model_signature")
                  continue
                ...
                prediction_matching_ratio = utils_tanh.check_prediction_matching_ratio(t_ws=weights,
                                                                                  t_bs=biases,
                                                                                  h_ws=cur_hat_ws,
                                                                                  h_bs=hat_bs)
    ...
    result = identify_final_model_candidate(surviving_models=survive_models)
    if result is None:
        print("No valid model candidates found. Extraction failed.")
        return
    pmr, ws_extracted, bs_extracted = result

    folder = './models/extracted_models/'
    tmp_path = ''
    for i in range(len(nn_shape)):
        tmp_path += '_' + str(nn_shape[i])
    # model_path = folder + 'tanh_2' + tmp_path
    model_path = folder + 'tanh_1' + tmp_path
    np.savez(model_path,
         ws=np.array(ws_extracted, dtype=object),
         bs=np.array(bs_extracted, dtype=object))
...
if __name__ == '__main__':
    ...
    nn_shape = [784, 2, 1]
    nn_path = './models/simulated/tanh_mnist_hidden2_1.npz'
